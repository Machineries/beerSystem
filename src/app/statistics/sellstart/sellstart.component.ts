...
@Component({
  selector: 'app-sellstart',
  templateUrl: './sellstart.component.html',
  styleUrls: ['./sellstart.component.css']
})

export class SellstartComponent {
  data: any = null;
  ...
  private setStatisticsData = (result: ISystembolagerResult) => {
      if (result.status === EEmitterStatus.Loaded) {
          const sortiment = result.data.artiklar.map(a => a.Saljstart);
